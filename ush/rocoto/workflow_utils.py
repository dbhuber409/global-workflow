...
def detectMachine():
    ...
    machines = ['HERA', 'S4', 'ORION', 'WCOSS_C', 'WCOSS_DELL_P3']
...
def create_wf_task(task, cdump='gdas', cycledef=None, envar=None, dependency=None, \
    ...
    if get_scheduler(detectMachine()) in ['slurm'] and detectMachine() in ['ORION', 'S4']:
...
def get_resources(machine, cfg, task, reservation, cdump='gdas'):
    ...
    if machine in [ 'WCOSS_DELL_P3', 'HERA', 'S4', 'ORION']:
    ...
    if machine in ['HERA', 'ORION', 'S4', 'WCOSS_C', 'WCOSS_DELL_P3']:
